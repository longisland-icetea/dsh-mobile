...
function installControl(): { remove: () => void; toggle: () => void; isOpen: () => boolean } {
  ...
  const renderWsDot = (): void => {
    ...
    if (pending.length === 0 || wsBlockedTotal(wsPathsBlocked) <= wsPathsSeenAttempts) return
    for (const trigger of document.querySelectorAll('.dsh-mobile-control__trigger')) {
      const dot = element('span', 'dsh-mobile-control__ws-dot'); dot.textContent = String(pending.length); dot.title = t('wsPathsNewBlocked'); dot.setAttribute('aria-hidden', 'true')
