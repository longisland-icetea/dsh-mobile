...
>
> **0.3.13 更新**：任务通知覆盖所有远程页面（网页通知，#46，浏览器可用；App 受系统后台策略限制）+ 第三方 WebSocket 拦截即通用一键放行（诊断页 + 红点，#47）。[详细记录](CHANGELOG.md)。
>
...
- **任务通知**：任务完成/失败/需审批时通知你——浏览器（含宽屏桌面与手机远程页面）在页面后台时弹系统通知；Android App 内经 WebMessage 桥交给壳弹原生通知（已实现，但受系统后台策略限制：激进的省电策略可能在 App 后台冻结页面直到再次打开）。长任务判定与 dsh-messager 语义一致（单轮 ≥7 次工具调用），短对话与子会话不打扰（#46）。
