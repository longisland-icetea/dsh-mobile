...
>
> **0.3.13 更新**：任务通知（浏览器系统通知，覆盖手机与宽屏远程页面，#46）+ 第三方 WebSocket 拦截即通用一键放行（诊断页 + 红点，#47）。[详细记录](CHANGELOG.md)。
>
> **升级提醒**：Windows DSH Desktop 用户请更新至插件 0.3.13；现有 0.3.3-0.3.12 App 可继续使用，无需重新配对。[兼容说明](#兼容性)。
...
- **第三方插件 WebSocket 一键放行**：诊断页按目录分组记录被拦截的插件连接（含次数），点允许即放行确切路径，未批准的一律拦截；有新拦截时侧栏入口与面板内「诊断」按钮出现红点计数提醒（#47）。若某插件的连接一直失败（如终端报 1006），先到诊断页看看有没有被拦的连接，一键放行即可，通常无需手动配置。
- **任务通知**：任务完成/失败/需审批时通知你——在页面后台（`document.hidden`）时弹浏览器系统通知，覆盖手机与宽屏桌面远程页面；长任务判定与 dsh-messager 语义一致（单轮 ≥7 次工具调用），短对话与子会话不打扰（#46）。
...
> **远程任务完成通知**：dsh-mobile 本身会在任务完成/失败/需审批时向远程页面推送浏览器系统通知（页面在后台时弹出，覆盖手机浏览器与宽屏桌面远程页）。浏览器通知权限按地址分别授权，远程地址需在浏览器中单独允许一次；Android App 的 WebView 无法显示浏览器通知，App 内不会弹。需要更可靠的推送时，也可配置第三方通道（如 dsh-messager 的飞书/企业微信/Telegram），它们走服务端下发，不受网关影响。
...
现有 0.3.3-0.3.12 App 无需重新配对；更早的 App 使用不同的状态栏策略，建议同步升级；App 0.1.3 及更早版本需卸载重装并重新配对。
