...
通过 npm 安装的插件会在桌面界面加载时检查新版本，有更新时在访问面板标题右侧显示“更新插件”，安装后需重启 DSH。App 下载入口展示最新版本；本地开发包不会被自动覆盖，Android App 暂无主动更新通知。更新前先看电脑端 DeepSeek Harness 的版本：**dsh-mobile 0.3.13 配套电脑端 0.1.3-alpha.2 及以上**；电脑端还是更早版本（0.1.2 线或 0.1.3-alpha.1）时，请保持 dsh-mobile 0.3.12。升级电脑端后请同步升级插件；升级插件后若手机页面异常，先核对电脑端版本。
