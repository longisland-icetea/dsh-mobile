...
>
> **0.3.13 更新**：第三方插件 WebSocket 一键放行（诊断页，#47）+ 面板修整（FRP 步骤编号、部署说明排版、批准按钮）。[详细记录](CHANGELOG.md)。
>
...
- **第三方插件 WebSocket 一键放行**：诊断页按目录分组记录被拦截的插件连接（含次数），点允许即放行确切路径，未批准的一律拦截；有新拦截时侧栏红点提醒（#47）。
- **手机页任务通知**：任务完成/失败/需审批时，手机弹系统通知——浏览器里走页面通知，Android App 内经 WebMessage 桥交给 App 弹原生通知（新增通知渠道与 Android 13 权限）（#46）。
