...
>
> **0.3.13 update**: task notifications via browser system notifications on every remote page (#46) plus interception-driven, plugin-agnostic WebSocket approval (diagnostics + red badge, #47). [Details](CHANGELOG.md).
>
...
- **One-click approval for third-party plugin WebSockets**: the diagnostics view groups blocked plugin connections by directory (with attempt counts); allowing a path unblocks that exact path while everything unapproved stays blocked, and a red count badge marks both the sidebar entry and the in-panel diagnostics button until reviewed (#47). If a plugin keeps failing to connect (for example a terminal reporting 1006), first look at the diagnostics view for blocked connections and approve them in one click — manual configuration is usually unnecessary.
- **Task notifications**: get notified when a task completes, fails, or needs approval — the page raises a browser system notification while it is hidden, covering phone and wide-desktop remote pages alike. Long-task detection matches dsh-messager semantics (at least seven tool calls in one turn); short chats and subagent chatter stay silent (#46).
...
> **Remote task-completion notifications**: dsh-mobile itself pushes browser system notifications for task completion, failure, and approval to the remote page (raised while the page is in the background — phone browsers and wide-desktop remote pages alike). Browser notification permission is granted per origin, so allow it once for the remote address; the Android app's WebView cannot display browser notifications, so nothing pops inside the app. If you need delivery you can fully rely on, a third-party channel (e.g. dsh-messager's Feishu/WeCom/Telegram push) is delivered server-side and unaffected by the gateway.
